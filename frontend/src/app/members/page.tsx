    ...
    <main className="page-content" style={{ backgroundColor: "var(--bg-page)" }}>
      ...
      <div className="container pt-4">
          ...
          <div className="members-header">
              <h1 className="members-title">Гишүүд</h1>
          ...
          <div className="members-layout">
              ...
              <div className="m-left-col">
                  ...
                  <div className="m-cards-grid">
                      {members.length === 0 && (
                          <div className="m-card" style={{ gridColumn: "1 / -1", padding: "2rem", textAlign: "center" }}>
                      ...
                      {members.map((member, index) => {
                          ...
                          desc = desc ? (desc.length > 220 ? desc.substring(0, 220) + "..." : desc) : 'Танилцуулга мэдээлэл оруулаагүй байна.';
                          const photoUrl = member.photo ? mediaUrl(member.photo) : null;

                          return (
                              <div
                                  key={member.id}
                                  className="m-card"
                                  style={index === 0 ? { borderColor: "var(--brand-primary)", boxShadow: "var(--shadow-md)" } : {}}
                              >
                                  {/* Large cover photo / initials hero */}
                                  <div className="m-card-cover">
                                      <SafeImage
                                          className="m-card-cover-img"
                                          src={photoUrl}
                                          alt={name}
                                          loading="lazy"
                                          fallback={
                                              <span className="m-card-cover-initials">
                                                  {mLogo(member.name, member.company)}
                                              </span>
                                          }
                                      />
                                      {/* */}
                                      <div className="m-card-cover-overlay" />
                                      <div className="m-card-badges">
                                          {index < 2 && (
                                              <span className="m-badge top-match">
                                                  <i className="fa-solid fa-star" /> Top Match
                                              </span>
                                          )}
                                          {isFeatured && (
                                          ...
                                          <span className="m-badge bni">BNI</span>
                                      </div>
                                  </div>

                                  <div className="m-card-body">
                                      <h3 className="m-card-title">{name}</h3>
                                      <div className="m-card-sector">
                                          <i className="fa-solid fa-building" /> {industryName}
                                      </div>
                                      <div className="m-card-loc">
                                          <i className="fa-solid fa-location-dot" /> {locationText}
                                      </div>
                                      ...
                                      <div className="m-card-actions">
                                          <Link href={`/member/${member.id}`} className="m-card-btn outline">
                                              Профайл
                                          </Link>
                                          <a
                                              href={
                      ...
                      <div>
                          ...
                          <div className="featured-slider-card position-relative">
                              {featuredMembers.map(fMember => (
                                  <Link
                                      key={fMember.id}
                                      href={`/member/${fMember.id}`}
                                      className="f-slider-item"
                                  >
                                      <div className="f-slider-logo">
                                          <SafeImage
                                              src={fMember.photo ? mediaUrl(fMember.photo) : ""}
                                              alt={fMember.company || fMember.name}
                                              loading="lazy"
                                              fallback={<>{mLogo(fMember.name, fMember.company)}</>}
                                          />
                                      </div>
                                      <div className="fw-bold text-truncate w-100">{fMember.company || fMember.name}</div>
                              ...
                              {recentMembers.map(rMember => (
                                  <div key={rMember.id} className="recent-conn-item">
                                      <div className="rc-logo">
                                          <SafeImage
                                              src={rMember.photo ? mediaUrl(rMember.photo) : ""}
                                              alt={rMember.company || rMember.name}
                                              loading="lazy"
                                              fallback={<>{mLogo(rMember.name, rMember.company)}</>}
                                          />
                                      </div>
              ...
              <aside className="m-right-col">
                  {preview ? (
                      <div className="profile-preview-card">
                          {/* Decorative banner with badges + window controls */}
                          <div className="pp-banner">
                              <div className="pp-top-bar">
                                  ...
                                  <div className="d-flex gap-2">
                                      <button className="btn btn-link p-0" aria-label="Expand">
                                          <i className="fa-solid fa-expand" style={{ fontSize: "0.7rem" }}></i>
                                      </button>
                          ...
                          <div className="pp-content">
                              {/* Centered, oversized profile photo lifted over the banner */}
                              <div className="pp-header">
                                  <div className="pp-logo">
                                      <SafeImage
                                          src={preview.photo ? mediaUrl(preview.photo) : ""}
                                          alt={preview.company || preview.name || ""}
                                          fallback={<>{mLogo(preview.name, preview.company)}</>}
                                      />
                                  </div>
                                  <div>
                                      <h2 className="pp-title">{preview.company || preview.name || 'Мэдээлэл алга'}</h2>
                                      <div className="pp-info-line">
                                          <i className="fa-solid fa-building" />
                                          <span>{preview.industry || 'Салбар тодорхойгүй'}</span>
                                      </div>
                                      <div className="pp-info-line">
                                          <i className="fa-solid fa-location-dot" />
                                          <span>{preview.position || 'Монгол'}</span>
                                      </div>
                                      {preview.website && (
                                          <a
                                              href={preview.website.startsWith("http") ? preview.website : `https://${preview.website}`}
                                              className="pp-link mt-1"
                                              target="_blank"
                                              rel="noopener noreferrer"
                                          >
                                              <i className="fa-solid fa-globe" /> {preview.website}
                                          </a>
                                      )}
                                  </div>
                              </div>
                              ...
                              <ul className="pp-list">
                                  ...
                                  <li><i className="fa-regular fa-circle-check text-primary"></i> BNI түншлэл</li>
                              ...
                              <div className="pp-main-actions">
                                  <Link
                                      href={`/member/${preview.id}`}
                                      className="btn-brand pp-main-btn text-decoration-none"
                                      style={{ display: "inline-flex", alignItems: "center", justifyContent: "center" }}
                                  >
                                      Профайл үзэх
                                  </Link>
                                  <button className="btn btn-light px-3" style={{ border: "1px solid var(--border-color)", background: "transparent" }} aria-label="Save">
                      ...
                      <div className="profile-preview-card d-flex align-items-center justify-content-center text-muted" style={{ minHeight: "400px", padding: "2rem" }}>
      ...
      <div className="m-bottom-stats-container">
          <div className="container">
              <h3 className="members-subtitle mb-4">Ерөнхий үзүүлэлт</h3>
